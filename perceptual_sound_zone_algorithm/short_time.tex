...
% To conclude \autoref{ch:perceptual_sound_zone:stft} a vector notation alternative representation of the short-time 
...
% To this end, let $\tilde{\vec{t}}^{(m)}_\mu\in\Complex^{N_w + N_h - 1}$ and 
% $\tilde{\vec{p}}^{(m)}_\mu\in\Complex{N_w + N_h - 1}$ 
% denote vectors that contain the target and achieved sound pressures $\tilde{T}^{(m)}[k, \mu]$ and 
...
% From this, a vector based variant of the the short-time frequency-domain pressure matching optimization problem 
